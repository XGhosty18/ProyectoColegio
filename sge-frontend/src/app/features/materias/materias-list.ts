...
import { Materia } from '../../core/models/types';
import { Modal } from '../../shared/modal';

@Component({
  selector: 'sge-materias-list',
  imports: [FormsModule, Modal],
  template: `
    <div class="page-header">
      <h1>Materias</h1>
      <button class="btn btn-primary" (click)="nuevo()">Nueva Materia</button>
    </div>
    @if (loading) { <div class="loading">Cargando...</div> }
    <div class="table-wrap">
      <table>
        <thead><tr><th>ID</th><th>Código</th><th>Nombre</th><th>Tipo</th><th>Horas Sem.</th><th></th></tr></thead>
        <tbody>
          @for (m of paginaActual; track m.id) {
            <tr>
              <td>{{ m.id }}</td><td>{{ m.codigo }}</td><td>{{ m.nombre }}</td><td>{{ m.tipo }}</td><td>{{ m.horasSemanalesReq ?? '—' }}</td>
              <td class="actions">
                <button class="btn-sm" (click)="editar(m)">Editar</button>
                <button class="btn-sm btn-danger" (click)="eliminar(m)">Eliminar</button>
              </td>
            </tr>
          } @empty { <tr><td colspan="6" class="empty">No hay materias</td></tr> }
        </tbody>
      </table>
    </div>
    @if (totalPaginas > 1) {
    ...
    <sge-modal [open]="showForm()" [title]="editando() ? 'Editar Materia' : 'Nueva Materia'" (close)="cerrarForm()">
      <form (ngSubmit)="guardar()" class="form">
        ...
        <div class="field"><label>Horas Semanales</label><input type="number" [(ngModel)]="form.horasSemanalesReq" name="horas"></div>
...
export class MateriasList implements OnInit {
  ...
  saving = false;
  form: Partial<Materia> = {};
  private editId?: number;
  ngOnInit() { this.cargar(); }
  cargar() { this.loading = true; this.api.get<Materia[]>('/materias').subscribe(r => { this.list = r; this.loading = false; }); }
  nuevo() { this.editId = undefined; this.editando.set(false); this.form = {}; this.showForm.set(true); }
  editar(m: Materia) { this.editId = m.id; this.editando.set(true); this.form = { ...m }; this.showForm.set(true); }
  cerrarForm() { this.showForm.set(false); }
  guardar() {
    this.saving = true;
    const obs = this.editId ? this.api.put<Materia>('/materias', this.editId, this.form) : this.api.post<Materia>('/materias', this.form);
    obs.subscribe({ next: () => { this.cargar(); this.cerrarForm(); this.saving = false; }, error: () => { alert('Error'); this.saving = false; } });
  }
  eliminar(m: Materia) { if (confirm(`¿Eliminar "${m.nombre}"?`)) this.api.delete('/materias', m.id!).subscribe({ next: () => this.cargar() }); }
}
