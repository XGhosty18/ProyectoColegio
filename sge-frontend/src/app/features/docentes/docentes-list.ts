...
import { Docente } from '../../core/models/types';
import { Modal } from '../../shared/modal';

@Component({
  selector: 'sge-docentes-list',
  imports: [FormsModule, Modal],
  template: `
    <div class="page-header">
      <h1>Docentes</h1>
      <button class="btn btn-primary" (click)="nuevo()">Nuevo Docente</button>
    </div>
    @if (loading) { <div class="loading">Cargando...</div> }
    <div class="table-wrap">
      <table>
        <thead><tr><th>ID</th><th>Código</th><th>Nombres</th><th>Apellidos</th><th>DNI</th><th>Especialidad</th><th></th></tr></thead>
        <tbody>
          @for (d of paginaActual; track d.id) {
            <tr>
              <td>{{ d.id }}</td><td>{{ d.codigoEmpleado }}</td><td>{{ d.nombres }}</td><td>{{ d.apellidos }}</td><td>{{ d.dni }}</td><td>{{ d.especialidad }}</td>
              <td class="actions">
                <button class="btn-sm" (click)="editar(d)">Editar</button>
                <button class="btn-sm btn-danger" (click)="eliminar(d)">Eliminar</button>
              </td>
            </tr>
          } @empty { <tr><td colspan="7" class="empty">No hay docentes</td></tr> }
        </tbody>
      </table>
    </div>
    @if (totalPaginas > 1) {
    ...
    <sge-modal [open]="showForm()" [title]="editando() ? 'Editar Docente' : 'Nuevo Docente'" (close)="cerrarForm()">
      <form (ngSubmit)="guardar()" class="form">
        <div class="field"><label>Código Empleado</label><input [(ngModel)]="form.codigoEmpleado" name="codigo" required></div>
        ...
        <div class="field"><label>Especialidad</label><input [(ngModel)]="form.especialidad" name="especialidad" required></div>
...
export class DocentesList implements OnInit {
  ...
  saving = false;
  form: Partial<Docente> = {};
  private editId?: number;
  ngOnInit() { this.cargar(); }
  cargar() { this.loading = true; this.api.get<Docente[]>('/docentes').subscribe(r => { this.list = r; this.loading = false; }); }
  nuevo() { this.editId = undefined; this.editando.set(false); this.form = {}; this.showForm.set(true); }
  editar(d: Docente) { this.editId = d.id; this.editando.set(true); this.form = { ...d }; this.showForm.set(true); }
  cerrarForm() { this.showForm.set(false); }
  guardar() {
    this.saving = true;
    const obs = this.editId ? this.api.put<Docente>('/docentes', this.editId, this.form) : this.api.post<Docente>('/docentes', this.form);
    obs.subscribe({ next: () => { this.cargar(); this.cerrarForm(); this.saving = false; }, error: () => { alert('Error'); this.saving = false; } });
  }
  eliminar(d: Docente) { if (confirm(`¿Eliminar a ${d.nombres} ${d.apellidos}?`)) this.api.delete('/docentes', d.id!).subscribe({ next: () => this.cargar() }); }
}
